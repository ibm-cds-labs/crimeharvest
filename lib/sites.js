...
var vegas = {
  ...
  mappingFile: 'http://ibm-cds-labs.github.io/open-data/crime/vegas_types.csv',
  ...
  process: function(site, data, mappings) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      // location
      if (thecrime.location_1) { //(thecrime.map_x && thecrime.map_y) {
...
var nola = {
  ...
  resource: 'ahnh-7f2h.json',
  mappingFile: 'http://ibm-cds-labs.github.io/open-data/crime/NOLAcrimecodes.csv',
  select: ['nopd_item','type_','typetext','mapx','mapy','timeclosed','block_address','location'],
  where: function() {
    var today = new Date();
    var yesterday = new Date();
    yesterday.setDate(today.getDate() - 1);
    ...
    return 'timeclosed>=\"2016-09-28\" AND timeclosed<\"2016-10-18\"';
  },
  process: function(site, data, mappings) {
    var newcrimes = [];
    for (var i = 0; i < data.length; i++) {
      ...
      var catcode = categorization(mappings, 'Type', thecrime.type_);
