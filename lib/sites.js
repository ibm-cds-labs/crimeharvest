...
var boston = {
  ...
  select: ['incident_number','offense_code','offense_code_group','offense_description','district','reporting_area','shooting','occurred_on_date','year','month','day_of_week','ucr_part','street'],
  where: function() {
    var today = new Date();
    var yesterday = new Date();
    yesterday.setDate(today.getDate() - 1);
  ...
  process: function(site, data, mappings) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (thecrime.lat && thecrime.long) {
        rec.geometry = {
          type: 'Point',
          coordinates: [ thecrime.long, thecrime.lat ]
        };
      }
