...
var vegas = {
  ...
  select: ['event_number','event_date','type','type_description','general_location','beat','disposition','map_x','map_y'],
  where: function() {
    var today = new Date();
    var yesterday = new Date();
    yesterday.setDate(today.getDate() - 1);
    ...
    return 'event_date>=\"2016-09-01\" AND event_date<\"2016-09-26\"';
  },
  process: function(site, data, mappings) {
    var newcrimes = [];
    for (var i = 0; i < data.length; i++) {
      ...
      // location
      if (thecrime.map_x && thecrime.map_y) {
...
module.exports = [philly, chicago, batonrouge, sf, boston, vegas];
